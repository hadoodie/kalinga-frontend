// src/components/logistics/registry/overview/MetricCards/OperationalAssets.jsx
...
export default function OperationalAssets() {
  ...
  // ✅ FIX: Memoize operationalAssets to avoid new array reference each render
  const operationalAssets = useMemo(
    () => assets.filter(asset => asset.status === 'Operational'),
    [assets]
  );

  useEffect(() => {
    applyFilters();
  }, [operationalAssets, filters, searchQuery]);

  const applyFilters = () => {
    let filtered = operationalAssets;
  ...
  const categoryOptions = ["All Categories", ...new Set(operationalAssets.map(asset => asset.category))];
  const locationOptions = ["All Locations", ...new Set(operationalAssets.map(asset => asset.location))];
  ...
  const activeAssets = operationalAssets.length;
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-green-900">Operational Assets</h1>
          <p className="text-gray-600 mt-2">All actively deployed and functioning assets</p>
        </div>
        ...
        <div className="bg-white rounded-xl p-6 border border-gray-200 mb-6">
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            <div className="flex-1 enhanced-search-container">
              <input
                type="text"
                placeholder="Search operational assets by ID, type, location, or personnel..."
                value={searchQuery}
              ...
              <div className="flex items-center justify-between">
                <div>
                  <span className="text-yellow-800 text-sm font-medium">
                    Showing {filteredAssets.length} of {operationalAssets.length} operational assets
                  </span>
                </div>
                <button
                  onClick={clearAllFilters}
