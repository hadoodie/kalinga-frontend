// src/components/logistics/registry/overview/MetricCards/UnassignedAssets.jsx
...
export default function StandbyAssets() {
  ...
  // FIXED: Filter for Standby assets instead of Unassigned
  const standbyAssets = assets.filter(asset => asset.status === 'Standby');

  useEffect(() => {
    applyFilters();
  }, [standbyAssets, filters, searchQuery]);

  const applyFilters = () => {
    let filtered = standbyAssets;
  ...
  const categoryOptions = ["All Categories", ...new Set(standbyAssets.map(asset => asset.category))];
  const locationOptions = ["All Locations", ...new Set(standbyAssets.map(asset => asset.location))];
  ...
  const standbyCount = standbyAssets.length;
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-green-900">Standby Assets</h1>
          <p className="text-gray-600 mt-2">Assets currently on standby and available for deployment</p>
        </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mb-8">
          ...
          >
            ...
            ></div>
            <div className="metric-tooltip">View Standby Assets</div>
            ...
            <div className="enhanced-metric-content flex items-center justify-between mb-4">
              ...
              <div className="text-right relative">
                <div className="absolute inset-0 bg-black/40 backdrop-blur-md rounded-lg -m-2 z-0"></div>
                <div className="relative z-10">
                  <p className="text-xs sm:text-xl font-medium text-green-600 mb-1">Standby</p>
                  <p className="text-3xl sm:text-8xl font-bold text-white">{standbyCount}</p>
                </div>
              </div>
            </div>
            <div className="enhanced-metric-content bg-white/20 rounded-lg p-2 sm:p-3 mt-auto backdrop-blur-sm">
              <p className="text-xs text-white font-medium">Available for deployment</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl p-6 border border-gray-200 mb-6">
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            <div className="flex-1 enhanced-search-container">
              <input
                type="text"
                placeholder="Search standby assets by ID, type, location, or personnel..."
                value={searchQuery}
              ...
              <div className="flex items-center justify-between">
                <div>
                  <span className="text-yellow-800 text-sm font-medium">
                    Showing {filteredAssets.length} of {standbyAssets.length} standby assets
                  </span>
                </div>
                <button
                  onClick={clearAllFilters}
