...
const Toast = ({ message, type = "success", onClose }) => {
  ...
  return (
    <div
      className={`fixed top-6 right-6 ${bgColor} text-white px-6 py-4 rounded-xl shadow-2xl z-50 animate-slide-in flex items-center gap-3 min-w-[320px] max-w-md`}
    >
      {type === "success" && (
...
const Modal = ({
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-md flex justify-center items-center p-4 z-[100] animate-fade-in">
      <div
        className={`bg-white rounded-2xl shadow-2xl w-full ${sizeClasses[size]} max-h-[90vh] overflow-y-auto border-2 border-green-300/50`}
      >
...
const Badge = ({
  ...
  return (
    <span
      className={`inline-flex items-center rounded-full px-3 py-1.5 text-sm font-medium ${variants[variant]} ${pulse ? "animate-pulse" : ""} ${className}`}
    >
...
const Card = ({
  ...
  <div
    className={`bg-white rounded-2xl shadow-lg ${border ? "border border-green-300/50" : ""} ${hover ? "hover:shadow-2xl hover:border-green-400 transition-all duration-300 hover:-translate-y-1" : ""} ${elevated ? "shadow-xl" : ""} ${className}`}
  >
    {children}
  </div>
);
...
const CardContent = ({ children, className = "", padding = true }) => (
  <div className={`${padding ? "p-6" : ""} ${className}`}>{children}</div>
);
...
const Button = ({
  ...
  const baseClasses =
    "inline-flex items-center justify-center rounded-xl font-medium transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed shadow-md hover:shadow-lg active:scale-95";

  const variants = {
    primary:
      "bg-gradient-to-r from-green-700 via-green-600 to-emerald-700 hover:from-green-600 hover:via-green-500 hover:to-emerald-600 text-white focus:ring-green-500",
    secondary:
      "bg-gradient-to-r from-green-500 to-emerald-500 hover:from-green-400 hover:to-emerald-400 text-white focus:ring-green-400",
    outline:
      "border-2 border-green-300 bg-white hover:bg-green-50 text-green-900 focus:ring-green-300 hover:border-green-400 hover:text-green-800",
    danger:
      "bg-gradient-to-r from-red-600 to-rose-600 hover:from-red-500 hover:to-rose-500 text-white focus:ring-red-500",
    success:
      "bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white focus:ring-emerald-500",
    ghost:
      "bg-transparent hover:bg-green-100 text-gray-700 border border-gray-300 hover:border-green-300 hover:text-green-800",
    info: "bg-gradient-to-r from-blue-600 to-cyan-600 hover:from-blue-500 hover:to-cyan-500 text-white focus:ring-blue-500",
    warning:
      "bg-gradient-to-r from-amber-500 to-yellow-500 hover:from-amber-400 hover:to-yellow-400 text-white focus:ring-amber-500",
  };
...
const InfoChip = ({
  ...
  const colors = {
    ...
    purple:
      "bg-gradient-to-r from-purple-50 to-violet-50 text-purple-800 border border-purple-200",
  ...
  return (
    <div
      className={`flex items-center gap-3 rounded-xl ${sizes[size]} ${colors[color]} transition-all duration-200 hover:scale-[1.02] hover:shadow-sm`}
    >
      {Icon && (
        <Icon
    ...
    <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
      <div>
        ...
        <p className="text-sm text-gray-600">
          Source to destination navigation with live tracking
        </p>
      </div>
      <div className="flex gap-3">
        ...
        <InfoChip
          label="Est. Duration"
          value={estimated_eta || "Calculating..."}
          icon={Clock}
          color="blue"
        />
      </div>
...
function getSourceHospital(allocation, assignmentDetails) {
  return (
    assignmentDetails?.sourceHospital ?? allocation?.sourceHospital ?? null
  );
}

function getDestinationHospital(allocation, assignmentDetails) {
  return (
    assignmentDetails?.destinationHospital ??
    allocation?.destinationHospital ??
    null
  );
}
...
function parseCapabilitiesShared(capabilitiesString) {
  ...
  try {
    ...
    if (
      typeof capabilitiesString === "string" &&
      (capabilitiesString.startsWith("[") || capabilitiesString.startsWith("{"))
    ) {
...
const AssignmentSummaryModal = ({
  ...
  const destinationHospital = getDestinationHospital(
    allocation,
    assignmentDetails,
  );
  ...
  const responderCapabilities = parseCapabilities(
    responder?.handling_capabilities,
  );
            ...
            <div className="flex flex-col md:flex-row items-center gap-6">
              ...
              <div className="flex-1 text-center md:text-left">
                ...
                <p className="text-green-800 text-lg">
                  Allocation #{allocation?.id} is now ready for live tracking
                  and dispatch
                </p>
              </div>
              <Badge
                variant="success"
                className="text-lg px-5 py-2.5 animate-pulse"
              >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
                ...
                <CardContent>
                  <div className="space-y-4">
                    <div className="flex justify-center mb-4">
                      <Badge
                        variant="success"
                        className="text-base px-5 py-2.5 animate-pulse"
                      >
                        <CheckCircle size={18} className="mr-2" />
                        {ALLOCATION_STATUS_UI_LABELS[allocation?.status] ||
                          allocation?.status}
                      </Badge>
                    </div>
                    ...
                    <InfoChip
                      ...
                      icon={AlertCircle}
                      color={
                        allocation?.request?.urgency_level === "Critical"
                          ? "danger"
                          : "warning"
                      }
                    />
            ...
            <div className="lg:col-span-3 space-y-6">
                      ...
                      <div
                        ...
                        onClick={() => {
                          if (vehicle && onViewVehicleDetails) {
                            console.log(
                              "Vehicle clicked in summary modal:",
                              vehicle,
                            );
                          ...
                          <div>
                            ...
                            <div className="w-full bg-gray-200 rounded-full h-2.5">
                              <div
                                ...
                                style={{
                                  width: `${vehicle?.current_fuel_level || 75}%`,
                                }}
                              ></div>
                            </div>
                          </div>
                    ...
                    <div className="space-y-4">
                      <div
                        ...
                        onClick={() => {
                          if (responder && onViewResponderProfile) {
                            console.log(
                              "Responder clicked in summary modal:",
                              responder,
                            );
                            ...
                            <div className="flex items-center justify-between">
                              <div>
                                ...
                                <p className="text-gray-700 font-medium">
                                  ...
                                  {responder?.years_experience || "5"} yrs exp
                                </p>
                              </div>
                              <Eye
                                size={20}
                          ...
                          {responder?.user?.address && (
                            <div className="flex items-start gap-2 text-sm">
                              ...
                              <div>
                                ...
                                {responder.user.barangay &&
                                  responder.user.city && (
                                    <p className="text-gray-600 text-xs">
                                      {responder.user.barangay},{" "}
                                      {responder.user.city}{" "}
                                      {responder.user.zip_code}
                                    </p>
                                  )}
                              </div>
                            </div>
                      ...
                      <div className="bg-white rounded-xl border border-gray-200 p-4">
                        ...
                        <div className="flex flex-wrap gap-2">
                          ...
                          {(!responder?.handling_capabilities ||
                            responderCapabilities.length === 0) && (
                            <span className="text-sm text-gray-500">
                              Standard handling
                            </span>
                          )}
              ...
              {[
                ...
                allocation?.allocationVehicles?.[0] && (
                  <Card
                    title="Assigned Logistics"
                    icon={Truck}
                    hover
                    elevated
                        ...
                        <div className="space-y-4">
                          ...
                          <div className="flex items-center gap-3 p-3 bg-blue-100/50 rounded-xl">
                            ...
                            <div className="flex-1">
                              ...
                              <p className="font-medium text-blue-900">
                                {formatPhone(responder?.contact_number)}
                              </p>
                            </div>
                          </div>
                          {responder?.user?.address && (
                          ...
                          <div className="space-y-4">
                            <div>
                              ...
                              <div className="grid grid-cols-2 gap-3">
                                ...
                                <InfoChip
                                  label="Year"
                                  value={vehicle?.year_manufactured}
                                  color="gray"
                                  size="sm"
                                />
                                <InfoChip
...
function EnhancedAllocationCard({
  ...
  const responderCapabilities = parseCapabilitiesShared(
    responder?.handling_capabilities,
  );
  ...
  const getStatusIcon = (status) => {
    switch (status) {
      ...
      case "in_transit":
        return <Navigation className="text-purple-600" size={20} />;
      case "delivered":
        return <PackageCheck className="text-emerald-600" size={20} />;
      default:
  ...
  return (
    <Card
      hover
      elevated
      className="border-l-4 border-l-green-500 hover:border-l-green-600 transition-all duration-300"
    >
      <CardContent className="p-6">
        ...
        <div className="flex items-start justify-between mb-6">
          <div className="flex items-center gap-4">
            <div
              className={`p-3 rounded-xl ${getStatusColor(allocation.status).split(" ")[0]}`}
            >
          ...
          <div
            className={`px-4 py-2 rounded-full border text-sm font-semibold ${getStatusColor(allocation.status)}`}
          >
            {statusLabel}
          </div>
        </div>
            ...
            <div>
              ...
              <div className="bg-gradient-to-r from-green-50 to-emerald-50 rounded-xl p-4 border border-green-200">
                ...
                <p className="text-xl text-gray-600">
                  {allocation.request?.resource_name ||
                    `Request #${allocation.request_id || "N/A"}`}
                </p>
              </div>
            </div>
          </div>
            ...
            <div className="bg-gradient-to-r from-blue-50 to-cyan-50 rounded-xl p-4 border border-blue-200">
              ...
              <div className="text-xl">
                <p className="font-semibold text-gray-900 truncate">
                  {allocation.sourceHospital?.name ||
                    allocation.source_hospital_id ||
                    "N/A"}{" "}
                  →{" "}
                  {allocation.destinationHospital?.name ||
                    allocation.destination_hospital_id ||
                    "N/A"}
                </p>
              </div>
            </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 pt-6 border-t border-gray-200">
          ...
          {hasLogisticsAssigned && (
            <Button
              onClick={() => onReadyToTransit?.(allocation)}
              variant="info"
              size="sm"
              icon={Truck}
              className="flex-1"
...
const StatusBadge = ({ status, className = "" }) => {
  ...
  return (
    <div
      className={`inline-flex items-center gap-2 px-4 py-2 rounded-full border font-semibold ${config.bg} ${config.text} ${config.border} ${className}`}
    >
      <Icon size={16} />
...
function IncomingRequestsPanel({ setActiveTab, updateCount }) {
  const { requests, setRequests } = useSessionAllocationsStore();
                ...
                <div className="space-y-3 p-2">
                  {filteredRequests.map((req) => (
                    <div
                      ...
                      className={`p-4 cursor-pointer transition-all duration-300 rounded-xl border ${
                        selected?.id === req.id
                    ...
                    >
                      ...
                      <div className="flex items-center justify-between text-sm text-gray-600 mb-3">
                        ...
                        <span>
                          {REQUEST_STATUS_UI_LABELS[req.status] || "Pending"}
                        </span>
                      </div>
          ...
          <Card hover elevated className="h-[700px] flex flex-col">
            <CardHeader className="bg-gradient-to-r from-green-50 to-emerald-50 border-b border-green-300">
              <div className="flex justify-between items-center">
                ...
                {selected && <Badge variant="default">ID: {selected.id}</Badge>}
              </div>
            </CardHeader>
              ...
              {selected ? (
                    ...
                    <div className="flex items-start justify-between">
                      <div>
                        ...
                        <div className="flex items-center gap-4 flex-wrap">
                          ...
                          <Badge
                            variant={
                              selected.urgency_level === "Critical"
                                ? "danger"
                                : selected.urgency_level === "High"
                                  ? "warning"
                                  : "success"
                            }
                          ...
                          <Badge
                            ...
                            className="text-base px-4 py-2"
                          >
                            {REQUEST_STATUS_UI_LABELS[selected.status] ||
                              "Pending"}
                          </Badge>
                        </div>
                      </div>
                        ...
                        <CardContent>
                          <div className="space-y-4">
                            ...
                            <div className="flex justify-between items-center py-2 border-b border-gray-200">
                              ...
                              <span className="font-semibold text-green-900">
                                {selected.handling_class || "General"}
                              </span>
                            </div>
                            <div className="flex justify-between items-center py-2">
                              <span className="text-gray-700 font-medium">
                      ...
                      <Card className="bg-gradient-to-br from-green-50 to-emerald-50 border-green-400 shadow-inner">
                        <CardContent>
                          <p className="text-gray-700 mb-6 text-sm leading-relaxed">
                            This request has been submitted and is now visible
                            to DOH Dispatchers. They will coordinate the
                            allocation from available hospitals using the
                          ...
                          <div className="space-y-4">
                            <div className="bg-gradient-to-r from-amber-50 to-yellow-50 border border-amber-200 rounded-xl p-4">
                              ...
                              <p className="text-sm text-amber-800 mt-1">
                                A dispatcher is reviewing all urgent requests
                                and will allocate resources shortly.
                              </p>
                            </div>
                ...
                <div className="flex flex-col items-center justify-center h-full text-gray-500 p-8">
                  ...
                  <p className="text-center text-gray-600 max-w-md">
                    Choose a request from the queue to view detailed information
                    and take allocation actions
                  </p>
                </div>
              )}
            </CardContent>
...
function EnhancedSessionAllocationsPanel({ showToast, updateCount }) {
  const {
    ...
    setSessionAllocations: setAllocations,
  } = useSessionAllocationsStore();
  ...
  useEffect(() => {
    console.log("Responder Modal State:", {
      showResponderModal,
      selectedResponder,
    });
  ...
  const fetchMyAllocations = async () => {
    try {
      ...
      const newCounts = {
        logistics_assigned: allocationsData.filter(
          (a) => a.status === "logistics_assigned",
        ).length,
        confirmed: allocationsData.filter((a) => a.status === "confirmed")
          .length,
        planned: allocationsData.filter((a) => a.status === "planned").length,
        other: allocationsData.filter(
          (a) =>
            !["logistics_assigned", "confirmed", "planned"].includes(a.status),
        ).length,
      ...
      allocationsData.forEach(async (alloc) => {
        if (
          ["logistics_assigned", "in_transit", "delivered"].includes(
            alloc.status,
          ) &&
          !assignmentDetailsMap[alloc.id]
        ) {
          try {
            const details = await allocationService.getAllocationDetails(
              alloc.id,
            );
            ...
            console.error(
              `Failed to fetch details for allocation ${alloc.id}:`,
              err,
            );
          }
        }
  ...
  const loadAssignmentDetails = async (allocationId, silent = false) => {
    ...
    try {
      ...
      const response =
        await allocationService.getAllocationDetails(allocationId);
  ...
  const handleReadyToTransit = async (allocation) => {
    try {
      if (!allocation.request_id) {
        showToast?.("Invalid allocation reference.", "error");
        return;
      }
      await allocationService.updateLogisticsStatus(
        allocation.request_id,
        "in_transit",
      );
      showToast?.("? Logistics is now in transit!", "success");
      fetchAllocations();
    } catch (e) {
      console.error(e);
      showToast?.("? Failed to transition to transit.", "error");
    }
  };
    ...
    return (
      <div className="flex flex-col items-center justify-center py-20">
        ...
        <p className="text-gray-500 mt-2">
          Fetching data from DOH Central Database
        </p>
      </div>
    );
  }
      ...
      <div className="bg-white rounded-2xl shadow-xl border border-green-300/50">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 p-8 border-b border-green-300/50">
          <div>
            ...
            <p className="text-gray-700 mt-1 text-lg">
              Live from DOH Central Database • {counts.total} total allocation
              {counts.total !== 1 ? "s" : ""}
            </p>
          </div>
          <div className="flex gap-3">
            <Button
              onClick={fetchMyAllocations}
              size="md"
              variant="outline"
              icon={RefreshCw}
            >
                ...
                return (
                  <EnhancedAllocationCard
                    ...
                    onViewResponderProfile={(responderData) => {
                      console.log(
                        "Responder clicked in card - data:",
        ...
        {allocations.length === 0 && (
          <Card className="border-2 border-dashed border-gray-300 hover:border-green-400 transition-all duration-300">
            <CardContent className="py-20 text-center">
              <PackageOpen
                size={80}
                className="mx-auto mb-6 text-green-600 opacity-50"
              />
              <h3 className="text-2xl font-bold text-green-900 mb-4">
                No Active Allocations
              </h3>
              <p className="text-lg text-gray-700 mb-6 max-w-md mx-auto">
                Create allocations in{" "}
...
export function Allocation() {
  const listenToLogistics = useSessionAllocationsStore(
    (state) => state.listenToLogistics,
  );

  useEffect(() => {
    // start listening to reverb when we mount Allocation
    listenToLogistics();
  }, [listenToLogistics]);
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-gray-700 mt-2 text-lg">
                Coordinate and manage resource allocations across DOH network
              </p>
            </div>
          </div>
          ...
          {activeTab === "tracking" && (
            <Card className="shadow-xl">
              <CardContent className="text-center py-12">
                ...
                <p className="text-gray-600">
                  Coming soon - Real-time tracking of all active allocations
                </p>
              </CardContent>
            </Card>
          )}
...
const CapabilityBadge = ({ capability }) => {
  const config = {
    ...
    Refrigerated: {
      color: "bg-gradient-to-r from-cyan-100 to-sky-100 text-cyan-800",
      icon: ThermometerSnowflake,
      label: "Refrigerated",
    },
    refrigerated: {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold ${cfg.color} shadow-sm`}
    >
      {Icon && <Icon size={12} />}
