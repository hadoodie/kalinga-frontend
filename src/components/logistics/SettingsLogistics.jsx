...
function PriorityAlertsModal({ onClose }) {
  const [alertSettings, setAlertSettings] = useState({
    low_stock: true,
    critical_requests: true,
    delayed_deliveries: true,
    damaged_items: true,
    near_expiry_items: true,
  });
  ...
  const alerts = [
    ...
    { key: "critical_requests", label: "Critical items" },
    { key: "damaged_items", label: "Damaged items" },
    ...
    <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex justify-center items-center z-50 p-4">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg p-6 border-t-4 border-green-700">
        ...
        <div className="space-y-3">
          <p className="text-sm font-semibold text-gray-700 mb-2">Alerts for:</p>
          {alerts.map((alert) => (
            <div
              key={alert.key}
    ...
    <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex justify-center items-center z-50 p-4">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg p-6 border-t-4 border-green-600">
        ...
        <div className="mb-4">
          ...
          />
          <p className="text-xs text-gray-500 mt-1">
            *This is a simplified editor. In production, this would be a specific form for "{setting}".
          </p>
        </div>
...
export default function LogisSettings() {
  ...
  const handleEdit = (setting, section) => {
    ...
    } else if (setting === "Priority Alerts") {
  ...
  const handleClosePriorityAlerts = () => {
  ...
  return (
    <div className="p-5 bg-gray-50 min-h-[calc(100vh-140px)] font-sans text-left">
      ...
      {showChangePassword && <ChangePasswordModal onClose={handleCloseChangePassword} />}
      {showMuteNotifications && <MuteNotificationsModal onClose={handleCloseMuteNotifications} />}
      {showPriorityAlerts && <PriorityAlertsModal onClose={handleClosePriorityAlerts} />}
    </div>
  );
}