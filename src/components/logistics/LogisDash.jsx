...
import { 
    Truck, Package, Clock, MapPin, List, AlertTriangle, ArrowRight, CornerDownRight 
} from 'lucide-react';
...
const getStatusColor = (status) => {
    switch (status) {
        ...
        case 'In Transit':
        case 'En Route': return 'text-green-600 bg-green-50';
    ...
    return (
        <div className="text-left p-5 bg-white rounded-2xl shadow-md border hover:shadow-xl transition flex flex-col">
            <div className="flex justify-between items-center">
                <h2 className="text-lg md:text-xl font-semibold text-gray-700">Pending Requests</h2>
                <List className="h-5 w-5 text-green-500" />
            </div>
            <p className="text-3xl md:text-5xl font-extrabold text-green-600 mt-3">{requests.length}</p>
...
const TotalResourcesSummary = ({ inventory }) => {
    ...
    return (
        <div className="text-left p-5 bg-white rounded-2xl shadow-md border hover:shadow-xl transition flex flex-col">
            <div className="flex justify-between items-center">
                <h2 className="text-lg md:text-xl font-semibold text-gray-700">Total Resource Items</h2>
                <Package className="h-5 w-5 text-green-500" />
            </div>
            <p className="text-3xl md:text-5xl font-extrabold text-green-600 mt-3">{inventory.length}</p>
            <div className="mt-3 text-sm">
                <p className="font-medium text-gray-600">{totalCategories} Distinct Categories</p>
...
const DeliveryStats = ({ shipments }) => {
    ...
    return (
        <div className="text-left p-5 bg-white rounded-2xl shadow-md border hover:shadow-xl transition flex flex-col">
            <div className="flex justify-between items-center">
                <h2 className="text-lg md:text-xl font-semibold text-gray-700">Delivery Performance</h2>
                <Clock className="h-5 w-5 text-highlight" />
            </div>
            <p className="text-3xl md:text-5xl font-extrabold text-highlight mt-3">1.5h</p>
            <div className="mt-3 text-sm">
    ...
    return (
        <div className="col-span-12 lg:col-span-4 bg-white rounded-2xl shadow-xl p-5 border border-gray-100 h-[450px] md:h-full flex flex-col">
            ...
            <div className="space-y-3 overflow-y-auto pr-2 flex-1">
                {shipments.map((s) => (
                    <div key={s.id} className={`p-3 rounded-xl border-l-4 ${s.status === 'Delayed' ? 'border-red-500 bg-red-50' : 'border-primary bg-green-50'} shadow-sm`}>
                        <div className="flex justify-between items-start">
                            <span className="font-semibold text-gray-800 flex items-center">
                                <span className={`text-xs font-bold px-2 py-0.5 rounded-full mr-2 ${getStatusColor(s.status)}`}>{s.status}</span>
                                {s.id}
                            </span>
                            <span className="text-xs font-bold text-gray-600">{formatETA(s.eta)} ETA</span>
                        </div>
                        ...
                        <p className="text-xs text-gray-500 mt-0.5 pl-5">Contents: {s.contents}</p>
                    </div>
                ))}
            </div>
            {shipments.length === 0 && (
...
const ResourceRequestsList = ({ requests }) => (
    <div className="col-span-12 bg-white rounded-2xl shadow-md border p-5">
                    ...
                    {requests.map(r => (
                        <tr key={r.id} className="hover:bg-green-50">
                            ...
                            <td className="px-3 py-2 text-center">
                                <span className={`px-2 py-1 rounded-full border ${getUrgencyColor(r.urgency)}`}>{r.urgency}</span>
                            </td>
        ...
        <div className="mt-3 text-right">
            <Link to="/requested-allocation" className="text-sm text-green-600 font-semibold hover:text-green-700 flex items-center justify-end">
