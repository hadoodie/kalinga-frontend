    ...
    <div className="fixed inset-0 flex justify-center items-center p-4 z-50 backdrop-blur-md bg-black/30">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto transform transition-all duration-300 flex flex-col">
        <div className="p-6 border-b flex justify-between items-center sticky top-0 bg-white z-10 rounded-t-2xl">
          <h2 className={`text-xl font-extrabold ${COLORS.primaryText}`}>
            {title}
          </h2>
          <button
            onClick={onClose}
          ...
          >
            <X size={24} className={COLORS.primaryText} />
          </button>
        </div>
...
const AppointmentCard = ({ appointment, onSelect }) => {
  const statusClasses =
    COLORS.status[appointment.status] || COLORS.status.Upcoming;
  ...
  return (
    <div
      className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 md:p-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 cursor-pointer transition duration-200 hover:shadow-lg hover:border-primary group"
    ...
    >
      <div className="flex items-center gap-4 w-full sm:w-auto border-b sm:border-b-0 pb-3 sm:pb-0">
        ...
        <div className="flex flex-col flex-1 text-left">
          <p
            className={`text-sm font-semibold flex items-center gap-1 ${COLORS.primaryText}`}
          >
            <Clock3 size={16} className="text-yellow-600" /> {appointment.time}
          </p>
          <p className={`text-lg font-bold ${COLORS.primaryText}`}>
      ...
      <div className="flex items-center gap-4 w-full sm:w-auto justify-between sm:justify-end pt-3 sm:pt-0">
        <span
          className={`inline-flex items-center px-3 py-1 text-xs font-bold rounded-full ${statusClasses}`}
        >
          {appointment.status}
        </span>
        <button
          className="p-2 rounded-full text-gray-400 group-hover:text-primary hover:bg-green-50 transition"
    ...
    <div className="flex flex-col text-left">
      ...
      {isLink ? (
        <a
          ...
          className="font-bold text-green-800 text-xs hover:text-primary hover:underline"
        >
          {value}
        </a>
      ) : (
  ...
  return (
    <div className="space-y-6 h-full flex flex-col">
      <div className="bg-green-50 p-5 rounded-xl border border-green-100 shadow-inner text-left">
        <h3 className="text-sm font-bold text-green-800 flex items-center gap-2 mb-2 uppercase tracking-wide">
          <QrCode size={18} /> Reference: {appointment.id}
        </h3>
        <p
          className={`text-2xl font-extrabold text-center ${COLORS.primaryText}`}
        >
          {appointment.hospital}
        </p>
      ...
      <div className="flex-grow">
        ...
        {appointment.instructions && (
          <div className="mt-6 p-4 bg-yellow-50 rounded-lg border border-yellow-100 text-left">
            <h4 className="font-bold text-yellow-800 flex items-center gap-2 mb-1 text-sm">
        ...
        <div className="pt-4 border-t mt-auto">
          <div className="flex flex-row gap-3">
            ...
            <button
              ...
              className={`flex-1 flex items-center justify-center gap-2 px-4 py-3 text-sm font-semibold rounded-xl transition text-white ${COLORS.danger}`}
            >
              Cancel Appointment
            </button>
          </div>
        ...
        <div className="flex flex-col items-center gap-6 p-4">
          ...
          <div className="flex gap-4 mt-4">
            <button
              ...
              className="px-6 py-2 rounded-xl font-bold text-primary bg-gray-100 hover:bg-gray-200 transition"
            >
              No, Keep Appointment
            </button>
            <button
              onClick={handleCancelConfirm}
              className="px-6 py-2 rounded-xl font-bold text-white bg-red-500 hover:bg-red-600 transition"
            >
              Yes, Cancel
            </button>
          </div>
...
const StepTerms = ({ formData, updateForm }) => (
  <div className="space-y-6">
    <div className="prose max-w-none text-sm text-gray-600 bg-gray-50 p-6 rounded-xl border border-gray-200 h-64 overflow-y-auto text-justify">
      ...
      <p className="mb-4">
        This appointment and scheduling system allocates slots on a first come,
        first served basis. There is no guarantee that a slot will always be
        available for a user's first choice.
      </p>
      <p className="mb-4">
        Users accept the responsibility for providing, checking and verifying
        the validity and accuracy of the information they provide on this
        system.
      </p>
      <p className="font-bold mb-2 text-primary">Sa Filipino</p>
      <p className="mb-4">
        Ang sistema ng appointment at pag-iskedyul na ito ay nagbibigay ng mga
        slot batay sa "first come, first served".
      </p>
      <p>
        Responsibilidad ng mga user ang magbigay, mag-check, at mag-verify ng
    ...
    <label className="flex items-start gap-3 p-4 border rounded-xl cursor-pointer hover:bg-gray-50 transition">
      ...
      <span className="text-sm text-gray-700">
        I have read and understood the instructions and information on this
        page, and agree to the Terms and Conditions. <br />
        <em className="text-primary text-xs">
          Nabasa at naunawaan ko ang mga instruksyon at impormasyon...
        </em>
      </span>
    </label>
  </div>
  ...
  <div className="space-y-6 max-w-2xl mx-auto py-4">
    <div className="space-y-2">
      ...
      >
        ...
        <option value="Jose R. Reyes Memorial Medical Center">
          Jose R. Reyes Memorial Medical Center
        </option>
        <option value="Dr. Jose Fabella Memorial Hospital">
...
const StepSchedule = ({ formData, updateForm }) => (
  <div className="flex flex-col md:flex-row gap-8">
    ...
    <div className="flex-1 border-l pl-0 md:pl-8 border-gray-200">
      <label className="text-sm font-bold text-primary mb-4 block">
      ...
      {formData.date ? (
        <div className="space-y-3 max-h-64 overflow-y-auto">
          {[
            ...
            <label
              key={time}
              className={`flex items-center justify-between p-3 rounded-lg border cursor-pointer transition ${formData.time === time ? "bg-primary text-white border-primary" : "hover:bg-gray-50 border-gray-200"}`}
            >
              ...
              <span
                className={`text-xs font-bold px-2 py-1 rounded ${formData.time === time ? "bg-white/20" : "bg-green-100 text-green-800"}`}
              >
  ...
  <div className="space-y-4">
    <div className="bg-yellow-50 p-4 rounded-xl mb-6">
      ...
      <p className="text-xs text-green-700">
        Please ensure all data matches your valid government ID.
      </p>
    </div>
        ...
        <div className="flex gap-4 mt-2 items-center">
          ...
          <label className="flex items-center gap-2">
            <input
              ...
              onChange={() => updateForm("sex", "Female")}
            />{" "}
            Female
          </label>
          <label className="flex items-center gap-2">
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-left">
      <InputField
        ...
        onChange={(v) => updateForm("civilStatus", v)}
      />
      <InputField
        label="Occupation"
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-left">
      ...
      <InputField
        ...
        onChange={(v) => updateForm("contactNo", v)}
        required
      />
    </div>

    <InputField
      label="Chief Complaint (Pangunahing sanhi ng pagbisita)"
      value={formData.complaint}
      onChange={(v) => updateForm("complaint", v)}
      required
      align="text-left"
    />
  </div>
...
const StepSummary = ({ formData }) => (
  <div className="space-y-6">
    <div className="bg-yellow-50 border border-yellow-200 p-4 rounded-xl text-sm text-yellow-800">
      <strong>Note:</strong> Please review your details carefully. You need to
      submit all information before the date is reserved.
    </div>
  ...
  <div className="flex flex-col items-center justify-center space-y-6 text-center py-4">
    ...
    <div className="w-full max-w-md bg-white border-2 border-dashed border-gray-300 p-6 rounded-xl space-y-4 relative">
      ...
      <div className="text-left space-y-2 text-sm">
        ...
        <p>
          ...
          <span className="font-bold">{formData.hospital}</span>
        </p>
        <p>
          <span className="font-bold text-primary block text-xs">Service:</span>{" "}
    ...
    <div className="text-left bg-gray-50 p-4 rounded-lg text-xs text-gray-600 border border-gray-200 w-full max-w-md">
      ...
      <ul className="list-disc pl-4 space-y-1">
        <li>
          We highly recommend to follow the scheduled appointment date and time.
        </li>
        <li>If email address is not available please SCREENSHOT this form.</li>
        <li>
          Please bring this form together with a valid government-issued ID.
        </li>
      </ul>
    </div>
    ...
    <button
      onClick={onClose}
      className={`w-full max-w-md py-3 text-white rounded-xl font-bold shadow-lg ${COLORS.primary}`}
    >
      Done
    </button>
  </div>
...
const BookingFlow = ({ onClose, onSuccess }) => {
  ...
  const handleNext = async () => {
    if (step === 1 && !formData.agreedToTerms)
      return alert("Please agree to the Terms.");
    if (step === 2 && (!formData.hospital || !formData.serviceType))
      return alert("Please select Hospital and Service.");
  ...
  return (
    <div className="flex flex-col h-full">
        ...
        <div className="flex justify-between items-center mb-6 px-2">
          {STEPS_LABELS.map((label, idx) => (
            <div key={idx} className="flex flex-col items-center flex-1">
              <div
                className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold transition-all duration-300 ${step > idx + 1 ? "bg-primary text-white" : step === idx + 1 ? "bg-primary text-white ring-4 ring-green-100" : "bg-gray-200 text-primary"}`}
              >
                {step > idx + 1 ? <CheckCircle size={14} /> : idx + 1}
              </div>
              <span
                className={`text-[10px] mt-1 font-semibold uppercase ${step === idx + 1 ? "text-primary" : "text-gray-400"} hidden sm:block`}
              >
                {label}
              </span>
            </div>
      ...
      <div className="flex-grow">
        {step === 1 && (
          <StepTerms formData={formData} updateForm={updateForm} />
        )}
        {step === 2 && (
          <StepServices formData={formData} updateForm={updateForm} />
        )}
        {step === 3 && (
          <StepSchedule formData={formData} updateForm={updateForm} />
        )}
        {step === 4 && (
          <StepPatientInfo formData={formData} updateForm={updateForm} />
        )}
      ...
      {step < 6 && (
        <div className="flex justify-between pt-6 mt-4 border-t border-gray-100">
          {step > 1 ? (
            <button
              onClick={handleBack}
              className="px-6 py-2.5 rounded-xl font-bold text-primary hover:bg-gray-100 transition flex items-center gap-2"
          ...
          >
            {isSubmitting ? "Processing..." : step === 5 ? "Submit" : "Next"}{" "}
            <ChevronRight size={20} />
          </button>
        </div>
      )}
    ...
    try {
      ...
      const formattedData = response.data.map((app) => {
        ...
        return {
          ...
          date: appDate,
          time: appDate.toLocaleTimeString("en-US", {
            hour: "2-digit",
            minute: "2-digit",
            hour12: true,
          }),
          ...
          status:
            (app.status || "upcoming").charAt(0).toUpperCase() +
            (app.status || "upcoming").slice(1),
  ...
  return (
    <div className="min-h-screen bg-background p-4 md:p-8 font-sans">
      <header className="mb-6 p-4 bg-background">
        <h1
          className={`text-3xl md:text-4xl font-extrabold text-primary text-left`}
        >
          Appointments
        </h1>
      </header>
        ...
        <section className="lg:w-7/12 flex-grow space-y-6">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-end mb-4 border-b border-gray-200 pb-0">
            ...
            <button
              ...
              className={`flex items-center gap-2 px-5 py-2 mb-2 font-bold rounded-xl text-white shadow-md hover:shadow-lg transform hover:-translate-y-1 transition-all ${COLORS.primary} text-sm`}
            >
          ...
          <div className="space-y-4">
            {isLoading ? (
              <p className="p-12 text-center text-gray-400 animate-pulse">
              ...
              <div className="p-12 text-center bg-white rounded-2xl border border-dashed border-gray-300 flex flex-col items-center justify-center text-gray-400">
                ...
                {activeTab === TABS.UPCOMING && (
                  <p
                    className="text-sm mt-2 text-primary cursor-pointer hover:underline"
                    onClick={() => setIsBookingModalOpen(true)}
                  >
                    Book one now?
                  </p>
          ...
          <div className="bg-white rounded-2xl shadow-xl border border-gray-100 p-6 transition duration-300 min-h-[500px]">
            {selectedAppointment ? (
              ...
              <div className="flex flex-col items-center justify-center h-full min-h-[450px] text-center p-8 opacity-60">
                ...
                <h3 className={`text-xl font-bold ${COLORS.primaryText}`}>
                  No Appointment Selected
                </h3>
                <p className="text-primary mt-2 max-w-xs">
                  Click on any appointment card on the left to view full
                  details, instructions, and actions.
                </p>
              </div>
            )}
          </div>
