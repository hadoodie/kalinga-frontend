...
export default function HealthRecords() {
      ...
      try {
        ...
        const mappedData = response.data.data.map((report) => {
          ...
          return {
            ...
            date: rDate
              ? new Date(rDate).toISOString().split("T")[0]
              : "1970-01-01",
  ...
  const paginatedTriage = filteredTriage.slice(
    (triagePage - 1) * TRIAGE_PAGE_SIZE,
    triagePage * TRIAGE_PAGE_SIZE,
  );
  ...
  const paginatedLabResults = labResults.slice(
          ...
          <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="flex flex-col text-left">
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      setTriageDateFrom(e.target.value);
                      setTriagePage(1);
                    }}
                ...
                <div className="flex flex-col text-left">
                  ...
                  <input
                    ...
                    value={triageDateTo}
                    onChange={(e) => {
                      setTriageDateTo(e.target.value);
                      setTriagePage(1);
                    }}
                    className="w-full p-2.5 rounded-lg text-sm bg-white border border-gray-300 focus:ring-2 focus:ring-green-700 outline-none transition"
                  />
                ...
                <div className="flex flex-col text-left">
                  ...
                  <select
                    value={triageSortBy}
                    onChange={(e) => {
                      setTriageSortBy(e.target.value);
                      setTriagePage(1);
                    }}
                    className="w-full p-2.5 rounded-lg text-sm bg-white border border-gray-300 focus:ring-2 focus:ring-green-700 outline-none transition"
                  >
            ...
            <div>
              {isTriageLoading ? (
                ...
                <div className="py-8 text-center text-red-500 font-medium bg-red-50 rounded-lg">
                  {triageError}
                </div>
                      ...
                      >
                        ...
                        <div className="space-y-2 text-sm">
                          ...
                          <div className="flex justify-between">
                            ...
                            <span className="capitalize px-2 py-0.5 bg-yellow-100 text-yellow-800 rounded text-xs font-bold">
                              {item.level}
                            </span>
                          </div>
                  ...
                  {(triageTotalPages > 1 || filteredTriage.length > 0) && (
                    <div className="flex flex-col-reverse md:flex-row justify-between items-center gap-4 mt-6">
                      <span className="text-xs text-gray-500">
                        Showing {paginatedTriage.length} of{" "}
                        {filteredTriage.length} records
                      </span>
                      {triageTotalPages > 1 && (
                        <div className="flex gap-2">
                          {Array.from(
                            { length: triageTotalPages },
                            (_, i) => i + 1,
                          ).map((pageNumber) => (
                            <PageIndicator
                              key={pageNumber}
                              number={pageNumber}
                              isCurrent={triagePage === pageNumber}
                              onClick={setTriagePage}
                            />
                          ))}
                        </div>
                      )}
          ...
          <div>
            ...
            <div>
              {isLoading ? (
                ...
                <>
                  ...
                  <div className="md:hidden space-y-4">
                    {paginatedLabResults.map((result, idx) => (
                      <div
                        key={idx}
                        className="border border-gray-200 rounded-lg p-4 shadow-sm bg-white hover:border-green-200 transition relative"
                      >
                        ...
                        <div className="flex justify-between items-center mb-3 pr-12">
                          {" "}
                          {/* pr-12 to avoid overlap with button */}
                  ...
                  {(labTotalPages > 1 || labResults.length > 0) && (
                    <div className="flex flex-col-reverse md:flex-row justify-between items-center gap-4 mt-6">
                      <span className="text-xs text-gray-500">
                        Showing {paginatedLabResults.length} of{" "}
                        {labResults.length} records
                      </span>
                      {labTotalPages > 1 && (
                        <div className="flex gap-2">
                          {Array.from(
                            { length: labTotalPages },
                            (_, i) => i + 1,
                          ).map((pageNumber) => (
                            <PageIndicator
                              key={pageNumber}
                              number={pageNumber}
                              isCurrent={labPage === pageNumber}
                              onClick={setLabPage}
                            />
                          ))}
                        </div>
                ...
                <div className="py-12 text-center text-gray-500 font-medium bg-gray-50 rounded-lg">
                  No results found.
                </div>
              )}
            </div>
